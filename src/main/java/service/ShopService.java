...
import model.*;
import repository.InvoiceListRepository;
...
public class ShopService {
    ...
    CustomerService PERSON_SERVICE = CustomerService.getInstance();
    ...
    private List<Device> createRandomListGoods() {
        ...
        deviceList.add(new Telephone(
        ));
        deviceList.add(new Television(
        ));
        return deviceList;
    }
        ...
        return data.stream()
                ...
                .peek(array -> {
                    for (String s : array) {
                        if (s.isBlank()) {
                            try {
                                throw new Exception("Incorrect line in file");
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
    ...
    private Device createGoods(final Map<String, String> mapFields) {
        Function<Map<String, String>, Device> mapper = map1 -> {
            ...
            DeviceTypes type = EnumUtils.getEnum(DeviceTypes.class, typeString);
            if (type == DeviceTypes.TELEPHONE) {
    ...
    private static Device createTelephone(final Map<String, String> map) {
        Telephone telephone = (Telephone) createDevice(map, DeviceTypes.TELEPHONE);
    ...
    private static Device createTelevision(final Map<String, String> map) {
        Television television = (Television) createDevice(map, DeviceTypes.TELEVISION);
    ...
    private static Device createDevice(final Map<String, String> map, final DeviceTypes deviceType) {
        final Device device;
        ...
        device.setDeviceTypes(deviceType);
